import { drawBlock } from "../BlingBlingAddons/util/render";
import Route from "../BlingBlingAddons/util/Route";
...
register('command', (message) => {
    if (Client.getMinecraft().field_71442_b.func_178889_l().func_77145_d()) { // if creative mode
        ...
        for (let i = 0; i < Route.getRoute().length; i++) {
            ChatLib.command(`setblock ${Route.getRoute()[i].x} ${Route.getRoute()[i].y} ${Route.getRoute()[i].z} ${message}`, false);
        }
    }
